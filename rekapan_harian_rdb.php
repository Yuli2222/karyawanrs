...
if (isset($_POST['update_keterangan'])) {
    ...
    // Update keterangan di absensi_rdb
    $stmt = $conn->prepare("UPDATE absensi_rdb SET keterangan=? WHERE id=?");
    $stmt->bind_param("si", $ket, $id);
    $stmt->execute();
    $stmt->close();

    // Update juga di rekapan_harian_rdb jika ada
    $q = $conn->query("SELECT nik, jabatan, tanggal, jenis_absen FROM absensi_rdb WHERE id='$id'");
    if ($q && $q->num_rows > 0) {
        $row = $q->fetch_assoc();
        ...
        $conn->query("UPDATE rekapan_harian_rdb SET keterangan='$ket_esc' 
...
if (isset($_POST['hapus_data'])) {
    ...
    // Hapus dari absensi_rdb
    $stmt = $conn->prepare("DELETE FROM absensi_rdb WHERE id=?");
    $stmt->bind_param("i", $id);
    $stmt->execute();
    $stmt->close();

    // Tidak perlu hapus manual di rekapan_harian_rdb jika hanya untuk laporan,
...
if (isset($_GET['export']) && $_GET['export'] == 'excel') {
    ...
    echo "<tr><th colspan='8' style='text-align:center; font-size:16px; background:#f2f2f2;'>Rekapan Harian RDB - $tanggal</th></tr>";
...
<head>
    <meta charset="UTF-8">
    <title>Rekapan Absensi Harian Klinik Rambutan Deli Binjai</title>
    <style>
...
<a href="absensi_rdb.php" class="nav-btn">⬅ Kembali</a>

<h2>Rekapan Absensi Harian Klinik Rambutan Deli Binjai</h2>
