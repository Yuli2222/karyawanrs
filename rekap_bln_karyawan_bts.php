...
$bulan = str_pad($bulan, 2, '0', STR_PAD_LEFT); // contoh: '08' bukan '8'
$tahun = isset($_GET['tahun']) ? intval($_GET['tahun']) : intval(date('Y'));
$export = isset($_GET['export']) ? $_GET['export'] : '';

// Untuk query gunakan integer
$bulanInt = intval($bulan);
$query = "SELECT * FROM karyawan_klinik_bts 
...
if ($export === 'excel') {
    ...
    header("Content-Disposition: attachment; filename=rekap_karyawan_klinik_Pratama_Batang_Serangan_(BTS){$bulan}{$tahun}.xls");
    echo "<meta http-equiv='Content-Type' content='text/html; charset=utf-8' />";
    echo "<table border='1'>
        <tr><th colspan='14'>Rekap Data Karyawan Klinik Pratama Batang Serangan (BTS) Bulan $bulan Tahun $tahun</th></tr>
        <tr>
    ...
    if ($result && $result->num_rows > 0) {
        while ($row = $result->fetch_assoc()) {
            // Format tanggal lahir & masuk (aman bila kosong)
...
<head>
    <meta charset="UTF-8">
    <title>Rekap Data Karyawan Bulanan Klinik Pratama Batang Serangan (BTS)</title>
    <link href="https://cdn.jsdelivr.net/npm/bootstrap@5.3.0/dist/css/bootstrap.min.css" rel="stylesheet">
    <!-- (Opsional) Font Awesome untuk ikon -->
...
<div class="container py-4">
    <h3 class="mb-4">Rekap Data Karyawan Bulanan Klinik Pratama Batang Serangan (BTS)</h3>
    ...
    <div class="mb-3 no-print d-flex gap-2">
        ...
        <a href="data_karyawan_k_bts.php" class="btn btn-secondary">Kembali</a>
        <!-- Tombol Tambah Karyawan -->
        <a href="tambah_data_bts.php?bulan=<?= $qs_bulan ?>&tahun=<?= $qs_tahun ?>" class="btn btn-primary ms-auto">
    ...
    <table class="table table-bordered table-sm align-middle">
        ...
        if ($result && $result->num_rows > 0) {
            while ($row = $result->fetch_assoc()) {
                ...
                $id   = htmlspecialchars($row['id_bts']);
                ...
                echo "<tr>
                        ...
                        <td class='text-center'>
                            <a href='edit_data_bts.php?id={$id}&bulan={$qs_bulan}&tahun={$qs_tahun}' class='btn btn-warning btn-sm' title='Edit'>
                                <i class='fas fa-edit'></i>
                            </a>
                            <a href='hapus_data_bts.php?id={$id}&bulan={$qs_bulan}&tahun={$qs_tahun}' 
