...
if (isset($_POST['update_keterangan'])) {
    ...
    // Update keterangan di absensi_bts
    $stmt = $conn->prepare("UPDATE absensi_bts SET keterangan=? WHERE id=?");
    $stmt->bind_param("si", $ket, $id);
    $stmt->execute();
    $stmt->close();

    // Update juga di rekapan_harian_bts jika ada
    $q = $conn->query("SELECT nik, jabatan, tanggal, jenis_absen FROM absensi_bts WHERE id='$id'");
    if ($q && $q->num_rows > 0) {
        $row = $q->fetch_assoc();
        ...
        $conn->query("UPDATE rekapan_harian_bts SET keterangan='$ket_esc' 
...
if (isset($_POST['hapus_data'])) {
    ...
    // Hapus dari absensi_bts
    $stmt = $conn->prepare("DELETE FROM absensi_bts WHERE id=?");
    $stmt->bind_param("i", $id);
    $stmt->execute();
    $stmt->close();

    // Tidak perlu hapus manual di rekapan_harian_bts jika hanya untuk laporan,
...
if (isset($_GET['export']) && $_GET['export'] == 'excel') {
    ...
    echo "<tr><th colspan='8' style='text-align:center; font-size:16px; background:#f2f2f2;'>Rekapan Harian BTS - $tanggal</th></tr>";
...
<head>
    <meta charset="UTF-8">
    <title>Rekapan Absensi Harian Klinik Pratama Batang Serangan (BTS)</title>
    <style>
...
<a href="absensi_bts.php" class="nav-btn">⬅ Kembali</a>

<h2>Rekapan Absensi Harian Klinik Pratama Batang Serangan (BTS)</h2>
