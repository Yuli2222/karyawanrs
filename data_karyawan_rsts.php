...
// Fungsi untuk memeriksa apakah data sudah ada (cek semua field inti)
function isDataExists($conn, $nik, $nama, $jenis_kelamin, $tanggal_lahir, $alamat, $agama, $pendidikan_terakhir) {
    ...
    $query = "SELECT 1 FROM karyawan_rsts
              WHERE nik = '$nik' 
                AND nama = '$nama' 
...
// Proses tambah data (tetap kompatibel jika form simpan dipakai)
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['simpan_rsts'])) {
    ...
    // Periksa apakah data inti sudah ada
    if (isDataExists($conn, $nik, $nama, $jenis_kelamin, $tanggal_lahir, $alamat, $agama, $pendidikan_terakhir)) {
        echo "<div class='alert alert-warning mt-4'>Data karyawan dengan NIK " . htmlspecialchars($nik) . " sudah ada, tidak ditambahkan lagi.</div>";
    } else {
        $query = "INSERT INTO karyawan_rsts
        ...
        if ($conn->query($query) === TRUE) {
            ...
            echo "<div class='alert alert-danger mt-4'>Error: " . htmlspecialchars($query) . "<br>" . $conn->error . "</div>";
        }
    }
}

// Ambil parameter pencarian nama dan filter dari URL (GET)
$search_nama = isset($_GET['search_nama']) ? $conn->real_escape_string($_GET['search_nama']) : '';
$filter = isset($_GET['filter']) ? $_GET['filter'] : 'all'; // all | active | nonactive | cuti

// Build SQL filter berdasarkan pilihan (berdasarkan kolom status_kerja)
$filter_sql = '';
if ($filter === 'active') {
    // hanya status_kerja = 'Aktif'
    $filter_sql = "AND LOWER(TRIM(k.status_kerja)) = 'aktif'";
} elseif ($filter === 'nonactive') {
    // beberapa varian nonaktif/resign
    $filter_sql = "AND (LOWER(TRIM(k.status_kerja)) IN ('tidak aktif','non aktif','nonaktif','resign','keluar') OR LOWER(TRIM(k.status_kerja)) LIKE '%resign%' OR LOWER(TRIM(k.status_kerja)) LIKE '%nonaktif%')";
} elseif ($filter === 'cuti') {
    // menangkap 'cuti' dalam berbagai variasi
    $filter_sql = "AND LOWER(TRIM(k.status_kerja)) LIKE '%cuti%'";
} else {
    $filter_sql = ""; // all
}

// Kondisi pencarian nama (jika ada)
$name_sql = '';
if ($search_nama !== '') {
...
$tahun = isset($_GET['tahun']) ? $conn->real_escape_string($_GET['tahun']) : '';
...
// Query utama
if ($bulan !== '' || $tahun !== '') {
    // Jika ada filter bulan/tahun → tampilkan semua data sesuai tanggal_input
    $query = "
        SELECT k.*
        FROM karyawan_rsts k
        WHERE 1=1
        $filter_sql
        $name_sql
        $bulan_tahun_sql
    ...
    // Jika tanpa filter bulan/tahun → ambil data terakhir per NIK
    $query = "
        SELECT k.*
        FROM karyawan_rsts k
        INNER JOIN (
            SELECT nik, MAX(id_rsts) AS keep_id
            FROM karyawan_rsts
            GROUP BY nik
        ) t ON k.nik = t.nik AND k.id_rsts = t.keep_id
        WHERE 1=1
        $filter_sql
        $name_sql
        ORDER BY k.nama ASC
...
<head>
    ...
    <title>Data Karyawan Rumah Sakit Tanjung Selamat (RSTS)</title>
    <link href="https://cdn.jsdelivr.net/npm/bootstrap@5.3.0/dist/css/bootstrap.min.css" rel="stylesheet">
    <link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/6.4.0/css/all.min.css">
    <style>
        .form-check-label { margin-left: .25rem; }

        /* Tabel & action-bar styling — menyesuaikan tampilan PT.TDM */
...
<body>
    <div class="container mt-4">
        <h2> Data Karyawan - Rumah Sakit Tanjung Selamat (RSTS)</h2>

        <!-- FORM: pencarian + filter
             Form membungkus area pencarian, tombol filter, dan action-bar
             sehingga tombol filter masih submit GET (name="filter") -->
        ...
        ?>
            ...
            <div class="col-12">
                <div class="action-bar">
                    <a href="rekap_bln_karyawan_rsts.php" class="btn btn-outline-success">
                        <i class="fas fa-calendar-alt me-1"></i> Rekapan Bulanan
                    </a>

                    <a href="tambah_data_rsts.php" class="btn btn-success">
                        ...
                        if ($result && $result->num_rows > 0) {
                            ...
                            while($row = $result->fetch_assoc()) {
                                ...
                                echo "<tr class='small-cell'>
                                    ...
                                    <td class='td-actions'>
                                        <a href='edit_data_rsts.php?id=".htmlspecialchars($row['id_rsts'])."' class='btn btn-warning btn-sm' title='Edit'><i class='fas fa-edit'></i></a>
                                        <a href='hapus_data_rsts.php?id=".htmlspecialchars($row['id_rsts'])."' class='btn btn-danger btn-sm' onclick='return confirm(\"Yakin ingin menghapus data ini?\")' title='Hapus'><i class='fas fa-trash'></i></a>
                                    </td>
                                </tr>";
                                $no++;
                            }
