...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if (isset($_POST['single_save_masuk']) || isset($_POST['single_save_pulang'])) {
        ...
        $q_karyawan = $conn->query("SELECT * FROM karyawan_rsts WHERE nik='$nik_esc' AND jabatan='$jabatan_esc' LIMIT 1");
        ...
        $qcek = $conn->query("SELECT id FROM absensi_rsts
                              WHERE nik='$nik_esc' 
        ...
        if ($qcek && $qcek->num_rows > 0) {
            ...
            $id  = $row['id'];
            $sql_update = "UPDATE absensi_rsts SET keterangan=?, jam=? WHERE id=?";
            ...
            $qk = $conn->query("SELECT * FROM karyawan_rsts
                                WHERE nik='$nik_esc' 
                                AND jabatan='$jabatan_esc' 
                                ORDER BY id_rsts DESC LIMIT 1");
            ...
            $sql_insert = "INSERT INTO absensi_rsts(nik,nama,jabatan,foto,tanggal,jam,jenis_absen,keterangan,unit_kerja) 
                           VALUES(?,?,?,?,?,?,?,?,?)";
        ...
        $res_cek = $conn->query("SELECT id FROM rekapan_harian_rsts
                                 WHERE nik='$nik_esc' 
                                 AND jabatan='$jabatan_esc' 
                                 AND tanggal='$tanggal' 
                                 AND jenis_absen='$jenis_esc'");
        if ($res_cek && $res_cek->num_rows > 0) {
            $conn->query("UPDATE rekapan_harian_rsts
            ...
            $conn->query("INSERT INTO rekapan_harian_rsts(nik,nama,jabatan,tanggal,jenis_absen,keterangan,unit_kerja) 
                          VALUES('$nik_esc',
        ...
        header("Location: absensi_rsts.php?bulan=".$redir_bulan."&tahun=".$redir_tahun."&tanggal=".$redir_tanggal);
        exit;
    }
    else {
...
<head>
<meta charset="UTF-8" />
<title>ABSENSI RUMAH SAKIT TANJUNG SELAMAT (RSTS)</title>
...
<body>
<div class="container">
<h3 class="text-center mb-4">ABSENSI RUMAH SAKIT TANJUNG SELAMAT (RSTS)</h3>
...
<div class="mb-3">
    <a href="rekapan_harian_rsts.php" class="btn btn-outline-primary btn-custom">Rekapan Harian</a>
    <a href="rekapan_bulanan_rsts.php" class="btn btn-outline-success btn-custom">Rekapan Bulanan</a>
</div>
