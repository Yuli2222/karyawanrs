...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if (isset($_POST['single_save_masuk']) || isset($_POST['single_save_pulang'])) {
        ...
        $q_karyawan = $conn->query("SELECT * FROM karyawan_karpim_pttdm WHERE nik='$nik_esc' AND jabatan='$jabatan_esc' LIMIT 1");
        ...
        $qcek = $conn->query("SELECT id FROM absensi_karpim_pttdm
                              WHERE nik='$nik_esc' 
        ...
        if ($qcek && $qcek->num_rows > 0) {
            ...
            $id  = $row['id'];
            $sql_update = "UPDATE absensi_karpim_pttdm SET keterangan=?, jam=? WHERE id=?";
            ...
            $qk = $conn->query("SELECT * FROM karyawan_karpim_pttdm
                                WHERE nik='$nik_esc' 
                                AND jabatan='$jabatan_esc' 
                                ORDER BY id_karpim DESC LIMIT 1");
            ...
            $sql_insert = "INSERT INTO absensi_karpim_pttdm(nik,nama,jabatan,foto,tanggal,jam,jenis_absen,keterangan,unit_kerja) 
                           VALUES(?,?,?,?,?,?,?,?,?)";
        ...
        // ====== sinkronisasi ke rekapan_harian_karpim_pttdm======
        $res_cek = $conn->query("SELECT id FROM rekapan_harian_karpim_pttdm
                                 WHERE nik='$nik_esc' 
                                 AND jabatan='$jabatan_esc' 
                                 AND tanggal='$tanggal' 
                                 AND jenis_absen='$jenis_esc'");
        if ($res_cek && $res_cek->num_rows > 0) {
            $conn->query("UPDATE rekapan_harian_karpim_pttdm
            ...
            $conn->query("INSERT INTO rekapan_harian_karpim_pttdm(nik,nama,jabatan,tanggal,jenis_absen,keterangan,unit_kerja) 
                          VALUES('$nik_esc',
        ...
        header("Location: absensi_karpim.php?bulan=".$redir_bulan."&tahun=".$redir_tahun."&tanggal=".$redir_tanggal);
        exit;
    }
    else {
...
$sql_karyawan = "
    SELECT * FROM karyawan_karpim_pttdm
...
$sql_absensi = "
    SELECT * FROM absensi_karpim_pttdm
...
<head>
<meta charset="UTF-8" />
<title>Absensi Karyawan Pimpinan Kantor Direksi PT. TDM</title>
...
<body>
<div class="container">
<h3 class="text-center mb-4">ABSENSI KARYAWAN PIMPINAN KANTOR DIREKSI PT. TDM</h3>
...
<div class="mb-3">
    <a href="rekapan_harian_karpim.php" class="btn btn-outline-primary btn-custom">Rekapan Harian</a>
    <a href="rekapan_bulanan_karpim.php" class="btn btn-outline-success btn-custom">Rekapan Bulanan</a>
</div>
