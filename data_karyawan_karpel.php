...
function isDataExists($conn, $nama, $jenis_kelamin, $tanggal_lahir, $alamat, $agama, $pendidikan_terakhir, $tanggal_input) {
    ...
    $query = "SELECT 1 FROM karyawan_karpel_pttdm
              WHERE nama = '$nama'
...
// -----------------------------
// PROSES TAMBAH DATA (simpan_karpel)
// -----------------------------
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['simpan_karpel'])) {
    ...
    if (isDataExists($conn, $nama, $jenis_kelamin, $tanggal_lahir, $alamat, $agama, $pendidikan_terakhir, $tanggal_input)) {
        ...
        $query = "INSERT INTO karyawan_karpel_pttdm
...
// -----------------------------
// PROSES EDIT DATA (update_karpel)
// -----------------------------
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['update_karpel'])) {
    $id_karpel = (int)$_POST['id_karpel'];
    $nik = $conn->real_escape_string($_POST['nik']);
    $nama = $conn->real_escape_string($_POST['nama']);
    $jenis_kelamin = $conn->real_escape_string($_POST['jenis_kelamin']);
    $tanggal_lahir = $conn->real_escape_string($_POST['tanggal_lahir']);
    ...
    $query = "UPDATE karyawan_karpel_pttdm SET 
                nik='$nik',
                nama='$nama',
...
if (isset($_GET['hapus']) && is_numeric($_GET['hapus'])) {
    $id = (int)$_GET['hapus'];
    if ($conn->query("DELETE FROM karyawan_karpel_pttdm WHERE id_karpel='$id'") === TRUE) {
...
if ($use_latest_per_nik) {
    $from_sql = "
        karyawan_karpel_pttdm k
        INNER JOIN (
            SELECT nik, MAX(id_karpel) AS keep_id
            FROM karyawan_karpel_pttdm
            GROUP BY nik
        ) t ON k.nik = t.nik AND k.id_karpel = t.keep_id
    ";
} else {
    $from_sql = "karyawan_karpel_pttdm k";
}
...
<head>
  ...
  <title>Data Karyawan Pelaksana Kantor Direksi PT.TDM (KARPEL) - PT. Tembakau Deli Medica</title>
...
<div class="container mt-4">
  <h2>Data Karyawan Pelaksana Kantor Direksi PT.TDM (KARPEL)</h2>
  ...
  <form method="get" class="row g-2 mb-3">
    ...
    <div class="col-md-3">
      <a href="rekap_bln_karpel_pttdm.php" class="btn btn-info"><i class="fas fa-chart-bar"></i> Rekap Bulanan</a>
    </div>
    ...
    <div class="modal-content">
      <form method="POST">
        ...
        <div class="modal-footer">
          <button type="submit" name="simpan_karpel" class="btn btn-primary">
...
if ($result && $result->num_rows > 0) {
    ...
    while ($row = $result->fetch_assoc()) {
        ...
        <tr>
            ...
            <td>
                <button class="btn btn-warning btn-sm" 
                        ...
                        data-bs-target="#editModal<?= $row['id_karpel'] ?>">
                    <i class="fas fa-edit"></i>
                </button>
                <a href="?hapus=<?= $row['id_karpel'] ?>" 
        ...
        <div class="modal fade" id="editModal<?= $row['id_karpel'] ?>" tabindex="-1">
          <div class="modal-dialog modal-lg">
            <div class="modal-content">
              <form method="POST">
                <div class="modal-header">
                ...
                <div class="modal-body row g-2">
                  <input type="hidden" name="id_karpel" value="<?= $row['id_karpel'] ?>">
                ...
                <div class="modal-footer">
                  <button type="submit" name="update_karpel" class="btn btn-primary">
