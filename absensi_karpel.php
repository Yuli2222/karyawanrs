...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if (isset($_POST['single_save_masuk']) || isset($_POST['single_save_pulang'])) {
        ...
        $q_karyawan = $conn->query("SELECT * FROM karyawan_karpel_pttdm WHERE nik='$nik_esc' AND jabatan='$jabatan_esc' LIMIT 1");
        ...
        $qcek = $conn->query("SELECT id FROM absensi_karpel_pttdm
                              WHERE nik='$nik_esc' 
        ...
        if ($qcek && $qcek->num_rows > 0) {
            ...
            $id  = $row['id'];
            $sql_update = "UPDATE absensi_karpel_pttdm SET keterangan=?, jam=? WHERE id=?";
            ...
            $qk = $conn->query("SELECT * FROM karyawan_karpel_pttdm
                                WHERE nik='$nik_esc' 
                                AND jabatan='$jabatan_esc' 
                                ORDER BY id_karpel DESC LIMIT 1");
            ...
            $sql_insert = "INSERT INTO absensi_karpel_pttdm(nik,nama,jabatan,foto,tanggal,jam,jenis_absen,keterangan,unit_kerja) 
                           VALUES(?,?,?,?,?,?,?,?,?)";
        ...
        // ====== sinkronisasi ke rekapan_harian_karpel_pttdm======
        $res_cek = $conn->query("SELECT id FROM rekapan_harian_karpel_pttdm
                                 WHERE nik='$nik_esc' 
                                 AND jabatan='$jabatan_esc' 
                                 AND tanggal='$tanggal' 
                                 AND jenis_absen='$jenis_esc'");
        if ($res_cek && $res_cek->num_rows > 0) {
            $conn->query("UPDATE rekapan_harian_karpel_pttdm
            ...
            $conn->query("INSERT INTO rekapan_harian_karpel_pttdm(nik,nama,jabatan,tanggal,jenis_absen,keterangan,unit_kerja) 
                          VALUES('$nik_esc',
        ...
        header("Location: absensi_karpel.php?bulan=".$redir_bulan."&tahun=".$redir_tahun."&tanggal=".$redir_tanggal);
        exit;
    }
    else {
...
$sql_karyawan = "
    SELECT * FROM karyawan_karpel_pttdm
...
$sql_absensi = "
    SELECT * FROM absensi_karpel_pttdm
...
<head>
<meta charset="UTF-8" />
<title>Absensi Karyawan Pelaksana Kantor Direksi PT. TDM</title>
...
<body>
<div class="container">
<h3 class="text-center mb-4">ABSENSI KARYAWAN PELAKSANA KANTOR DIREKSI PT. TDM</h3>
...
<div class="mb-3">
    <a href="rekapan_harian_karpel.php" class="btn btn-outline-primary btn-custom">Rekapan Harian</a>
    <a href="rekapan_bulanan_karpel.php" class="btn btn-outline-success btn-custom">Rekapan Bulanan</a>
</div>
