...
function isDataExists($conn, $nik, $nama, $jenis_kelamin, $tanggal_lahir, $alamat, $agama, $pendidikan_terakhir) {
    ...
    $query = "SELECT 1 FROM karyawan_klinik_grd
              WHERE nik = '$nik' 
                AND nama = '$nama' 
...
// Proses tambah data (jika form simpan dipakai)
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['simpan_garuda'])) {
    ...
    $tanggal_bergabung = $conn->real_escape_string($_POST['tanggal_bergabung']); // yyyy-mm-dd
    $masa_kontrak = $conn->real_escape_string($_POST['masa_kontrak']);
    // sesuaikan tipe kolom tanggal_input = date
    ...
    if (isDataExists($conn, $nik, $nama, $jenis_kelamin, $tanggal_lahir, $alamat, $agama, $pendidikan_terakhir)) {
        ...
        $query = "INSERT INTO karyawan_klinik_grd
...
$search_nama_safe = $conn->real_escape_string($search_nama);

$filter = isset($_GET['filter']) ? $_GET['filter'] : 'all'; // values: all | active | nonactive | cuti
...
$name_sql = '';
if ($search_nama_safe !== '') {
    $name_sql = "AND k.nama LIKE '%$search_nama_safe%'";
}

// Filter Bulan & Tahun (tampilan dan SQL aman)
...
// Jika bulan/tahun dipilih -> tampilkan baris pada bulan/tahun tersebut (tanpa kunci "terbaru per NIK")
// Jika tidak pilih bulan & tahun -> pertahankan logika lama (ambil versi terbaru per NIK)
$use_latest_per_nik = (is_null($bulan_int) && is_null($tahun_int));

if ($use_latest_per_nik) {
    $from_sql = "
        karyawan_klinik_grd k
        INNER JOIN (
            SELECT nik, MAX(id_klinik_garuda) AS keep_id
            FROM karyawan_klinik_grd
            GROUP BY nik
        ) t ON k.nik = t.nik AND k.id_klinik_garuda = t.keep_id
    ";
} else {
    $from_sql = "karyawan_klinik_grd k";
}
...
<head>
    ...
    <title>DATA KARYAWAN KLINIK PRATAMA GARUDA</title>
...
<body>
    <div class="container mt-4">
        <h2> DATA KARYAWAN - KLINIK PRATAMA GARUDA</h2>
            ...
            <div class="col-md-3">
                ...
                <select name="bulan" id="bulan" class="form-select">
                    ...
                    for ($m=1; $m<=12; $m++) {
                        ...
                        $selected = ($val === $bulan_raw) ? "selected" : "";
...
?>
            ...
            <div class="col-12">
                <div class="action-bar">
                    <a href="rekap_bln_karyawan_grd.php" class="btn btn-outline-success">
                        <i class="fas fa-calendar-alt me-1"></i> Rekapan Bulanan
                    </a>

                    <a href="tambah_data_garuda.php" class="btn btn-success">
                        ...
                        if ($result && $result->num_rows > 0) {
                            ...
                            while($row = $result->fetch_assoc()) {
                                ...
                                echo "<tr class='small-cell'>
                                    ...
                                    <td class='td-actions'>
                                        <a href='edit_data_garuda.php?id=".htmlspecialchars($row['id_klinik_garuda'])."' class='btn btn-warning btn-sm' title='Edit'><i class='fas fa-edit'></i></a>
                                        <a href='hapus_data_garuda.php?id=".htmlspecialchars($row['id_klinik_garuda'])."' class='btn btn-danger btn-sm' onclick='return confirm(\"Yakin ingin menghapus data ini?\")' title='Hapus'><i class='fas fa-trash'></i></a>
                                    </td>
                                </tr>";
                                $no++;
                            }
