...
function isDataExists($conn, $nik, $nama, $jenis_kelamin, $tanggal_lahir, $alamat, $agama, $pendidikan_terakhir) {
    ...
    $query = "SELECT 1 FROM karyawan_klinik_bts
              WHERE nik = '$nik' 
                AND nama = '$nama' 
...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['simpan_bts'])) {
    ...
    if (isDataExists($conn, $nik, $nama, $jenis_kelamin, $tanggal_lahir, $alamat, $agama, $pendidikan_terakhir)) {
        ...
        $query = "INSERT INTO karyawan_klinik_bts
...
/* ===========================
   ...
   =========================== */
// simpan nilai mentah untuk tampilan (selected)
...
// Jika bulan/tahun dipilih -> tampilkan baris pada bulan/tahun tsb (tanpa kunci "terbaru per NIK")
...
if ($use_latest_per_nik) {
    $from_sql = "
        karyawan_klinik_bts k
        INNER JOIN (
            SELECT nik, MAX(id_bts) AS keep_id
            FROM karyawan_klinik_bts
            GROUP BY nik
        ) t ON k.nik = t.nik AND k.id_bts = t.keep_id
    ";
} else {
    $from_sql = "karyawan_klinik_bts k";
}
...
<head>
    ...
    <title>Data Karyawan Klinik Pratama Batang Serangan (BTS)</title>
...
<body>
    <div class="container mt-4">
        <h2> Data Karyawan - Klinik Pratama Batang Serangan (BTS)</h2>
...
?>
            ...
            <div class="col-12">
                <div class="action-bar">
                    <a href="rekap_bln_karyawan_bts.php" class="btn btn-outline-success">
                        <i class="fas fa-calendar-alt me-1"></i> Rekapan Bulanan
                    </a>

                    <a href="tambah_data_bts.php" class="btn btn-success">
                        ...
                        if ($result && $result->num_rows > 0) {
                            ...
                            while($row = $result->fetch_assoc()) {
                                ...
                                echo "<tr class='small-cell'>
                                    ...
                                    <td class='td-actions'>
                                        <a href='edit_data_bts.php?id=".htmlspecialchars($row['id_bts'])."' class='btn btn-warning btn-sm' title='Edit'><i class='fas fa-edit'></i></a>
                                        <a href='hapus_data_bts.php?id=".htmlspecialchars($row['id_bts'])."' class='btn btn-danger btn-sm' onclick='return confirm(\"Yakin ingin menghapus data ini?\")' title='Hapus'><i class='fas fa-trash'></i></a>
                                    </td>
                                </tr>";
                                $no++;
                            }
