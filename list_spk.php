...
// -----------------------------
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['tambah_spk'])) {
    ...
    if ($nik === '' || $nama === '' || $no_kontrak === '' || $unit_kerja === '' || $jenis_kontrak === '' || $jabatan === '' || $tanggal_mulai === '') {
        ...
        $stmt = $conn->prepare("INSERT INTO spk (nik, nama, no_kontrak, unit_kerja, jenis_kontrak, jabatan, tanggal_mulai, tanggal_selesai, tanggal_input) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)");
        if ($stmt) {
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['update_kontrak'])) {
    ...
    if ($id_edit <= 0 || $nik_edit === '' || $nama_edit === '' || $no_kontrak_edit === '' || $unit_kerja_edit === '' || $jenis_kontrak_edit === '' || $jabatan_edit === '' || $tanggal_mulai_edit === '') {
        ...
        if ($tanggal_input_edit === '') {
            $q = $conn->prepare("SELECT tanggal_input FROM spk WHERE id = ?");
            if ($q) {
        ...
        $sql_up = "UPDATE spk SET nik = ?, nama = ?, no_kontrak = ?, unit_kerja = ?, jenis_kontrak = ?, jabatan = ?, tanggal_mulai = ?, tanggal_selesai = ?, tanggal_input = ? WHERE id = ?";
...
if (isset($_GET['hapus_id'])) {
    ...
    if ($hapus_id > 0) {
        $check = $conn->prepare("SELECT id FROM spk WHERE id = ?");
        if ($check) {
            $check->bind_param("i", $hapus_id);
            $check->execute();
            $res_check = $check->get_result();
            if ($res_check && $res_check->num_rows > 0) {
                $del = $conn->prepare("DELETE FROM spk WHERE id = ?");
                if ($del) {
...
$sql = "SELECT * FROM spk WHERE MONTH(tanggal_input) = '$bulanAngka' AND YEAR(tanggal_input) = '$tahun'";
if ($unitFilter != '') {
...
if (isset($_GET['export']) && $_GET['export'] == '1') {
    ...
    $fileName = "kontrak_spk_{$bulan}_{$tahun}.xls";
    header('Content-Type: application/vnd.ms-excel');
    header("Content-Disposition: attachment; filename=\"$fileName\";");
    echo "<table border='1'>
        <tr><th colspan='9'>Rekap Data Kontrak SPK Bulan $bulan Tahun $tahun</th></tr>
        <tr>
            <th>No</th>
            <th>NIK</th>
            <th>Nama</th>
...
<html>
<head>
    <title>Daftar Kontrak SPK</title>
    <meta charset="utf-8">
...
<div class="container">
    ...
    <?php
    $sql_names = "SELECT nik, nama FROM spk WHERE MONTH(tanggal_input) = '$bulanAngka' AND YEAR(tanggal_input) = '$tahun'";
    if ($unitFilter != '') {
...
<div class="modal fade" id="modalTambah" tabindex="-1" role="dialog" aria-labelledby="modalTambahLabel" aria-hidden="true">
  <div class="modal-dialog modal-lg" role="document">
    <form method="POST" class="modal-content">
      <input type="hidden" name="tambah_spk" value="1">
      <div class="modal-header">
        <h5 class="modal-title" id="modalTambahLabel">Tambah Kontrak SPK</h5>
...
<div class="modal fade" id="modalEdit" tabindex="-1" role="dialog" aria-labelledby="modalEditLabel" aria-hidden="true">
  <div class="modal-dialog modal-lg" role="document">
    <form method="POST" class="modal-content">
      ...
      <div class="modal-header">
        <h5 class="modal-title" id="modalEditLabel">Edit Kontrak SPK</h5>
        <button type="button" class="close" data-dismiss="modal" aria-label="Tutup">
          <span aria-hidden="true">&times;</span>
        </button>
      </div>
