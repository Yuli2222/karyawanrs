...
function isDataExists($conn, $nik, $nama, $jenis_kelamin, $tanggal_lahir, $alamat, $agama, $pendidikan_terakhir) {
    ...
    $query = "SELECT 1 FROM karyawan_kantor_pttdm
              WHERE nik = '$nik' 
                AND nama = '$nama' 
...
// Proses tambah data
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['simpan_pttdm'])) {
    ...
    if (isDataExists($conn, $nik, $nama, $jenis_kelamin, $tanggal_lahir, $alamat, $agama, $pendidikan_terakhir)) {
        ...
        $query = "INSERT INTO karyawan_kantor_pttdm
...
if ($use_latest_per_nik) {
    $from_sql = "
        karyawan_kantor_pttdm k
        INNER JOIN (
            SELECT nik, MAX(id_pttdm) AS keep_id
            FROM karyawan_kantor_pttdm
            GROUP BY nik
        ) t ON k.nik = t.nik AND k.id_pttdm = t.keep_id
    ";
} else {
    $from_sql = "karyawan_kantor_pttdm k";
}
...
<head>
    ...
    <title>Data Karyawan KANTOR DIREKSI PT.TDM</title>
...
<body>
    <div class="container mt-4">
        <h2> Data Karyawan - KANTOR DIREKSI PT.TDM</h2>
        ...
        ?>
            ...
            <div class="col-12">
                <div class="action-bar">
                    <a href="rekap_bln_karyawan_pttdm.php" class="btn btn-outline-success">
                        <i class="fas fa-calendar-alt me-1"></i> Rekapan Bulanan
                    </a>

                    <a href="tambah_data_pttdm.php" class="btn btn-success">
                        ...
                        if ($result && $result->num_rows > 0) {
                            ...
                            while($row = $result->fetch_assoc()) {
                                ...
                                echo "<tr class='small-cell'>
                                    ...
                                    <td class='td-actions'>
                                        <a href='edit_data_pttdm.php?id=".htmlspecialchars($row['id_pttdm'])."' class='btn btn-warning btn-sm' title='Edit'><i class='fas fa-edit'></i></a>
                                        <a href='hapus_data_pttdm.php?id=".htmlspecialchars($row['id_pttdm'])."' class='btn btn-danger btn-sm' onclick='return confirm(\"Yakin ingin menghapus data ini?\")' title='Hapus'><i class='fas fa-trash'></i></a>
                                    </td>
                                </tr>";
                                $no++;
                            }
