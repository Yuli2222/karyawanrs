...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if (isset($_POST['single_save_masuk']) || isset($_POST['single_save_pulang'])) {
        ...
        $q_karyawan = $conn->query("SELECT * FROM karyawan_klinik_grd WHERE nik='$nik_esc' AND jabatan='$jabatan_esc' LIMIT 1");
        ...
        $qcek = $conn->query("SELECT id FROM absensi_garuda
                              WHERE nik='$nik_esc' 
        ...
        if ($qcek && $qcek->num_rows > 0) {
            ...
            $id  = $row['id'];
            $sql_update = "UPDATE absensi_garuda SET keterangan=?, jam=? WHERE id=?";
            ...
            $qk = $conn->query("SELECT * FROM karyawan_klinik_grd
                                WHERE nik='$nik_esc' 
                                AND jabatan='$jabatan_esc' 
                                ORDER BY id_klinik_garuda DESC LIMIT 1");
            ...
            $sql_insert = "INSERT INTO absensi_garuda(nik,nama,jabatan,foto,tanggal,jam,jenis_absen,keterangan,unit_kerja) 
                           VALUES(?,?,?,?,?,?,?,?,?)";
        ...
        // ====== sinkronisasi ke rekapan_harian_bts ======
        $res_cek = $conn->query("SELECT id FROM rekapan_harian_garuda
                                 WHERE nik='$nik_esc' 
                                 AND jabatan='$jabatan_esc' 
                                 AND tanggal='$tanggal' 
                                 AND jenis_absen='$jenis_esc'");
        if ($res_cek && $res_cek->num_rows > 0) {
            $conn->query("UPDATE rekapan_harian_garuda
            ...
            $conn->query("INSERT INTO rekapan_harian_garuda(nik,nama,jabatan,tanggal,jenis_absen,keterangan,unit_kerja) 
                          VALUES('$nik_esc',
        ...
        header("Location: absensi_garuda.php?bulan=".$redir_bulan."&tahun=".$redir_tahun."&tanggal=".$redir_tanggal);
        exit;
    }
    else {
...
$sql_karyawan = "
    SELECT * FROM karyawan_klinik_grd
...
<head>
<meta charset="UTF-8" />
<title>Absensi Klinik Pratama Garuda</title>
...
<body>
<div class="container">
<h3 class="text-center mb-4">ABSENSI KLINIK PRATAMA GARUDA</h3>
...
<div class="mb-3">
    <a href="rekapan_harian_garuda.php" class="btn btn-outline-primary btn-custom">Rekapan Harian</a>
    <a href="rekapan_bulanan_garuda.php" class="btn btn-outline-success btn-custom">Rekapan Bulanan</a>
</div>
